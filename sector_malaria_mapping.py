...
# Page configuration
st.set_page_config(
    page_title="Rwanda Malaria Sectors Dashboard",
    page_icon="🦟",
    layout="wide",
...
def create_choropleth_map(data, year, metric='Simple malaria cases'):
    ...
    if metric == 'Simple malaria cases':
        ...
        title = f'Simple Malaria Cases distribution- {year}'
        colorbar_title = 'Simple Malaria Cases'
    else:  # incidence
        color_column = 'incidence'
        title = f'Simple Malaria incidence - {year}'
    ...
    fig = px.choropleth_mapbox(
        ...
        mapbox_style='carto-darkmatter',  # Light gray canvas
        zoom=6.8,
...
def create_top_sectors_chart(data, year, metric='Simple malaria cases', top_n=10):
    ...
    if metric == 'Simple malaria cases':
        ...
        title = f'Top 10 sectors with most simple malaria cases - {year}'
    else:  # incidence
        y_title = 'Incidence'
...
def create_trend_chart(data, selected_sectors, metric='incidence'):
    ...
    if metric == 'Simple malaria cases':
        ...
        y_title = 'Simple Malaria Cases'
        title = 'Simple Malaria Cases Trends Over Time'
    else:  # incidence
        y_column = 'incidence'
        y_title = 'Incidence'
        title = 'Simple Malaria Rate Trends Over Time'
...
def create_province_scatterplot(data, year=2025):
    """Create scatterplot showing Population vs Incidence with Province as hue and quadrant annotations."""
    import plotly.express as px
    import numpy as np
    ...
    filtered_data = filtered_data[(filtered_data['Population'] >= 0) &
                                  (filtered_data['incidence'] >= 0)].copy()

    if filtered_data.empty:
        return None, None, None

    # Compute percentile thresholds (75th percentile)
    population_threshold = np.percentile(filtered_data['Population'], 75)
    incidence_threshold = np.percentile(filtered_data['incidence'], 75)

    # Axis limits with some padding - X axis starts at -100
    x_lower_bound = -100
    x_upper_bound = max(filtered_data['Population'].max() * 1.1, population_threshold * 1.5)
    y_upper_bound = max(filtered_data['incidence'].max() * 1.1, incidence_threshold * 1.5)
    ...
    fig = px.scatter(
        ...
        title='Sector Performance: Population vs Incidence (2025)',
        labels={
            'Population': 'Population',
            'incidence': 'Incidence (per 1,000 people)'
        },
        opacity=0.85
    )

    # Add quadrant lines
    fig.add_shape(type="line", x0=population_threshold, y0=0, x1=population_threshold, y1=y_upper_bound,
                  line=dict(color="white", width=1.5, dash="dot"))
    fig.add_shape(type="line", x0=x_lower_bound, y0=incidence_threshold, x1=x_upper_bound, y1=incidence_threshold,
                  line=dict(color="white", width=1.5, dash="dot"))
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            zeroline=False,
            color='white',
            range=[x_lower_bound, x_upper_bound],  # X axis now starts at -100
            title='Population'
        ),
        yaxis=dict(
            showgrid=True,
            ...
            title='Incidence (per 1,000 people)'
        ),
        legend=dict(
            title='Province',
    ...
    # Add quadrant labels - adjusted for new x-axis range
    fig.add_annotation(
        x=x_lower_bound + 0.05 * (x_upper_bound - x_lower_bound),
        y=incidence_threshold - 0.101 * y_upper_bound,
        text="Low pop<br>Low Incidence",
        showarrow=False,
        font=dict(color="lightgray", size=11),
        xanchor="left", yanchor="top"
    )
    fig.add_annotation(
        x=x_lower_bound + 0.755 * (x_upper_bound - x_lower_bound),
        y=incidence_threshold - 0.101 * y_upper_bound,
        text="High pop<br>Low Incidence",
        showarrow=False,
        font=dict(color="lightgray", size=11),
        xanchor="left", yanchor="top"
    )
    fig.add_annotation(
        ...
        text="Low Pop<br>High Incidence",
        showarrow=False,
        font=dict(color="lightgray", size=11),
        xanchor="left", yanchor="bottom"
    )
    fig.add_annotation(
        x=x_lower_bound + 0.75 * (x_upper_bound - x_lower_bound),
        y=0.75 * y_upper_bound,
        text="High Pop<br>High Incidence",
        showarrow=False,
        font=dict(color="lightgray", size=11),
        xanchor="left", yanchor="bottom"
    )

    return fig, population_threshold, incidence_threshold

def main():
    # Title and subtitle
    st.title("🏥 Rwanda Malaria Sector Dashboard")
    ...
    selected_year = st.sidebar.slider(
        ...
        value=max(years), 
        step=1,
        help="Choose a year to view malaria statistics"
    )
    
    # Metric selection
    metric_options = {
        'Simple Malaria Cases': 'Simple malaria cases',
        'Incidence (per 1,000 people)': 'incidence'
    }
    selected_metric_display = st.sidebar.selectbox(
        "📊 Primary Metric",
        list(metric_options.keys()),
        help="Choose the metric shown on the map and in comparison charts"
    )
    selected_metric = metric_options[selected_metric_display]
    
    st.sidebar.markdown("---")
    st.sidebar.subheader("📍 Sector Comparison")
    
    selected_sectors = st.sidebar.multiselect(
        "🏘️ Select Sectors for Comparison (Max 10)", 
        sector_options, 
        default=[], 
        max_selections=10,
        help="Select sectors to track trends over time. Format: Sector (District)"
    )
    ...
    with col1:
        st.metric(
            ...
            f"{total_cases:,.0f}",
            help="Total reported malaria cases across all sectors"
        )
    
    with col2:
        st.metric(
            "📊 National Incidence (per 1,000)", 
            f"{overall_incidence:.2f}",
            help="National average incidence of malaria per 1,000 population"
        )
    ...
    with col3:
        if change_percent is not None:
            ...
            <div style="background-color: #2b2b2b; border: 1px solid #444; padding: 0.5rem; border-radius: 0.25rem;">
                <div style="font-size: 14px; color: white;">📉 Incidence Change vs {previous_year}</div>
            ...
            st.metric("📉 Incidence Change", "No data available")
    
    # Map and Top Sectors
    st.subheader("🗺️ Sector Map & Top Sectors")
    ...
    # Sector analysis
    st.subheader("📈 Sector-Level Trends")
    col_left, col_right = st.columns([1, 1])
    
    with col_left:
        st.markdown("### 📊 Trends Over Time by Sector")
        if selected_sectors:
            trend_fig = create_trend_chart(data, selected_sectors, selected_metric)
            if trend_fig:
                st.plotly_chart(trend_fig, use_container_width=True)
        else:
            st.info("👈 Select sectors from the sidebar to view their trends over time")
            st.markdown("""
            **📋 How to Use This Chart:**
            1. Select sectors using the sidebar
            2. Compare trends over years
            """)
    
    with col_right:
        st.markdown("### 🎯 Sector Priority Zones (2025)")
        province_fig, pop_threshold, inc_threshold = create_province_scatterplot(data, year=2025)
        if province_fig:
